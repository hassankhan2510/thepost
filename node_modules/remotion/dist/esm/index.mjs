...
import { useContext as useContext16, useLayoutEffect as useLayoutEffect2, useRef as useRef5 } from "react";
...
var withInteractivitySchema = ({
  ...
  const Wrapped = forwardRef2((props, ref) => {
    ...
    const env = useRemotionEnvironment();
    const canUseRemotionHooks = useContext17(CanUseRemotionHooks);
...
var RegularSequenceRefForwardingFunction = ({
  ...
  const effectRuntimeValues = useMemo15(() => _remotionInternalEffects?.runtimeValues ?? null, [_remotionInternalEffects]);
...
import React17, { useCallback as useCallback8, useImperativeHandle, useMemo as useMemo18, useRef as useRef9 } from "react";
...
import { useEffect as useEffect4, useMemo as useMemo17, useRef as useRef8 } from "react";
...
var Canvas = React17.forwardRef(CanvasRefForwardingFunction);
...
// src/Artifact.tsx
import { useContext as useContext19, useLayoutEffect as useLayoutEffect6, useState as useState8 } from "react";
...
import { forwardRef as forwardRef7, useCallback as useCallback15, useContext as useContext31 } from "react";
...
// src/loop/index.tsx
import React18, { createContext as createContext18, useMemo as useMemo20 } from "react";
import { jsx as jsx17 } from "react/jsx-runtime";
var LoopContext = createContext18(null);
var useLoop = () => {
  return React18.useContext(LoopContext);
};
var Loop = ({
  durationInFrames,
  times = Infinity,
...
import { useContext as useContext22, useLayoutEffect as useLayoutEffect7, useRef as useRef15 } from "react";
...
import { useMemo as useMemo22, useRef as useRef12 } from "react";
var warned = false;
...
var BufferingContextReact = React22.createContext(null);
...
var useCurrentTimeOfMediaTagWithUpdateTimeStamp = (mediaRef) => {
  const lastUpdate = React23.useRef({
...
var AudioForDevelopmentForwardRefFunction = (props, ref) => {
  ...
  const effectToUse = React24.useInsertionEffect ?? React24.useLayoutEffect;
  effectToUse(() => {
    return () => {
      requestAnimationFrame(() => {
        cleanupOnMediaTagUnmount();
...
var HtmlInCanvasContent = forwardRef9(({
  ...
  const isInsideAncestorHtmlInCanvas = useContext32(HtmlInCanvasAncestorContext);
  ...
  const setLayoutCanvasRef = useCallback17((node) => {
  ...
  const onPaintCb = useCallback17(async () => {
...
var HtmlInCanvasInner = forwardRef9(({
  ...
  const setCanvasRef = useCallback17((node) => {
...
var CanvasImageContent = forwardRef10(({
  ...
  const continuePendingLoadDelay = useCallback18(({ markAsReady }) => {
  ...
  const canvasRef = useCallback18((canvas) => {
    setOutputCanvas(canvas);
    if (refForOutline) {
...
// src/IFrame.tsx
import { forwardRef as forwardRef11, useCallback as useCallback19, useState as useState17 } from "react";
...
var ImgContent = ({
  ...
  const imageCallbackRef = useCallback20((img) => {
  ...
  const actualSrc = usePreload(src);
  const retryIn = useCallback20((timeout) => {
    if (!imageRef.current) {
      return;
    }
...
// src/Interactive.tsx
import React31, { forwardRef as forwardRef12, useCallback as useCallback21, useRef as useRef26 } from "react";
...
var compositionsRef = React32.createRef();
...
var CompositionManagerProvider = ({
  ...
  const updateCompositions = useCallback22((updateComps) => {
  ...
  const registerComposition = useCallback22((comp) => {
  ...
  const unregisterComposition = useCallback22((id) => {
    setCompositions((comps) => {
      return comps.filter((c2) => c2.id !== id);
    });
  }, []);
  const registerFolder = useCallback22((name, parent, nonce, stack) => {
  ...
  const unregisterFolder = useCallback22((name, parent) => {
    setFolders((prevFolders) => {
  ...
  return /* @__PURE__ */ jsx31(CompositionManager.Provider, {
...
var RenderResourceManagerContext = React35.createContext(null);
...
var SequenceStackTracesUpdateContext = React36.createContext(() => {});
...
import React37, { createContext as createContext25 } from "react";
var CurrentScaleContext = React37.createContext(null);
...
var useCurrentScale = (options) => {
  const hasContext = React37.useContext(CurrentScaleContext);
  const zoomContext = React37.useContext(PreviewSizeContext);
  const config = useUnsafeVideoConfig();
  const env = useRemotionEnvironment();
  const [portalScale, setPortalScale] = React37.useState(getPortalNodeCurrentScale);
  React37.useEffect(() => {
...
import React38, { useContext as useContext36 } from "react";
var PixelDensityContext = React38.createContext(null);
...
var OffthreadVideoForRendering = ({
  ...
  const onImageFrame = useCallback23((img) => {
    if (onVideoFrame) {
      onVideoFrame(img);
    }
  }, [onVideoFrame]);
...
var VideoForDevelopmentRefForwardingFunction = (props2, ref) => {
  ...
  const effectToUse = React40.useInsertionEffect ?? React40.useLayoutEffect;
  effectToUse(() => {
    return () => {
      requestAnimationFrame(() => {
...
var InnerOffthreadVideo = (props2) => {
  ...
  const onDuration = useCallback25(() => {
    return;
  }, []);
...
import React42, { useMemo as useMemo39 } from "react";
import { jsx as jsx37 } from "react/jsx-runtime";
function useRemotionContexts() {
  const compositionManagerCtx = React42.useContext(CompositionManager);
  const timelineContext = React42.useContext(TimelineContext);
  const timelineImperativeContext = React42.useContext(TimelineImperativeContext);
  const setTimelineContext = React42.useContext(SetTimelineContext);
  const sequenceContext = React42.useContext(SequenceContext);
  const nonceContext = React42.useContext(NonceContext);
  const canUseRemotionHooksContext = React42.useContext(CanUseRemotionHooks);
  const preloadContext = React42.useContext(PreloadContext);
  const resolveCompositionContext = React42.useContext(ResolveCompositionContext);
  const renderAssetManagerContext = React42.useContext(RenderAssetManager);
  const sequenceManagerContext = React42.useContext(SequenceManager);
  const sequenceManagerRefContext = React42.useContext(SequenceManagerRefContext);
  const visualModePropStatusesRefContext = React42.useContext(VisualModePropStatusesRefContext);
  const bufferManagerContext = React42.useContext(BufferingContextReact);
...
var RemotionContextProvider = (props2) => {
  ...
  return /* @__PURE__ */ jsx37(LogLevelContext.Provider, {
    ...
    children: /* @__PURE__ */ jsx37(CanUseRemotionHooks.Provider, {
      ...
      children: /* @__PURE__ */ jsx37(NonceContext.Provider, {
...
var flattenChildren = (children) => {
  ...
  return childrenArray.reduce((flatChildren, child) => {
    if (child.type === React43.Fragment) {
...
var SeriesInner = (props2) => {
  const childrenValue = useMemo40(() => {
    ...
    const renderChildren = (i, startFrame) => {
      ...
      return React44.cloneElement(castedElement, {
...
var Still = (props2) => {
  ...
  return React45.createElement(Composition, newProps);
};
addSequenceStackTraces(Still);
// src/video/html5-video.tsx
import { forwardRef as forwardRef16, useCallback as useCallback26, useContext as useContext40 } from "react";
...
var VideoForwardingFunction = (props2, ref) => {
  ...
  const onDuration = useCallback26((src, durationInSeconds) => {
    setDurations({ type: "got-duration", durationInSeconds, src });
  }, [setDurations]);
